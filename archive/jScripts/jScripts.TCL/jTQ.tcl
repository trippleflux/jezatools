set jTQ(default_version)									"1.0.2.9"
...
set jTQ(ustats,stats)					""
set jTQ(user,ustats,mbup)				""
set jTQ(user,ustats,mbdn)				""
set jTQ(user,credits)					""
set jTQ(all,credits)						""
...
#
#
proc jTQ:CREDITS {nick uhost hand chan text} {
  
	global jTQ jSB jScripts
	
	if { !$jTQ(enable_credits) } { return }
	
	if { ([lindex $text 0] == "") } { 
		
		#jScripts:SND $chan [jTQ:FormatString $jTQ(skin,credits,help)]
		
		set status 	[catch { exec -- $jScripts(location_jScripts) CREDITSALL } result]
		
		if { $status > 0 } { 
			putlog "..jTQ...: $jScripts(location_jScripts) CREDITSALL FAiLED ($result)"
			jScripts:SND $jTQ(default_admin_chan) "..jTQ...: $jScripts(location_jScripts) CREDITSALL FAiLED ($result)"
			return
		}

		set jTQ(all,credits) [lindex $result 0]
		
		set output						[jTQ:FormatString $jTQ(skin,credits,all)]
		
		if { $jTQ(default_show_credits) == "chan" } 				{ jScripts:SND $chan $output }
		if { $jTQ(default_show_credits) == "private" }			{ jScripts:SND $nick $output }
		if { $jTQ(default_show_credits) == "notice" }			{ putquick "NOTICE $nick :$output" }

		return 
		
	}

	if { $jTQ(enable_debug_partyline) } { putlog "..jTQ...: jTQ:CREDITS {$nick $uhost $hand $chan $text}" }
	
	foreach section $jTQ(stats_sections_locale) {
		
		set n [split $section ':']
		
		set jTQ(user,found)	false
		
		set FILE [open "$jScripts(location_jTQ)files/[lindex $n 0].[lindex $n 1]" "r"]
		...
		foreach line $data {
			...
			;#found user
			if { [lindex $line 0] == [lindex $text 0] } {
				
				if { $jTQ(enable_debug_partyline) } { putlog "..jTQ...: [lindex $text 0] --> $line" }
				
				set jTQ(user,found)	true
				
				set jTQ(section_name)		[lindex $n 0]
				set jTQ(text_username)		[lindex $line 0]
				set jTQ(text_groupname)		[lindex $line 1]
				
				set jTQ(user,credits)		[expr [lindex $line 4]/1024]
				
				set output						[jTQ:FormatString $jTQ(skin,credits)]
		...
		#lets say we use only one credits section :)
		break
	...
	if { !$jTQ(user,found) } {
		...
		set jTQ(text_username) [lindex $text 0]
		
		set output						[jTQ:FormatString $jTQ(skin_user_not_found)]
...
#############
# !ustats login all/mn/wk/day
#
#
proc jTQ:USTATS {nick uhost hand chan text} {
  
	global jTQ jSB jScripts
	
	if { !$jTQ(enable_ustats) } { return }
	
	if { ([lindex $text 0] == "") || ([lindex $text 1] == "") } { 
		
		jScripts:SND $chan [jTQ:FormatString $jTQ(skin,ustats,help)]
		
		return 
		
	}

	if { $jTQ(enable_debug_partyline) } { putlog "..jTQ...: jTQ:USTATS {$nick $uhost $hand $chan $text}" }
	...
	foreach section $jTQ(stats_sections_locale) {
		...
		foreach line $data {
			...
			if { [lindex $line 0] == [lindex $text 0] } {
				...
				set jTQ(ustats,stats)		"ALL"
				set jTQ(user,ustats,mbup)	[expr [lindex $line end-1] / 1]
				set jTQ(user,ustats,mbdn)	[expr [lindex $line end] / 1]
				
				set jTQ(section_name)		[lindex $n 0]
				set jTQ(text_username)		[lindex $line 0]
				set jTQ(text_groupname)		[lindex $line 1]
				
				if { [string tolower [lindex $text 1]] == "all" } {
				...
				if { [string tolower [lindex $text 1]] == "mn" } {
					
					set jTQ(ustats,stats)		"MN"
					set jTQ(user,ustats,mbup)	[expr [lindex $line end-3] / 1]
					set jTQ(user,ustats,mbdn)	[expr [lindex $line end-2] / 1]
					
				}
				
				if { [string tolower [lindex $text 1]] == "wk" } {
					
					set jTQ(ustats,stats)		"WK"
					set jTQ(user,ustats,mbup)	[expr [lindex $line end-5] / 1]
					set jTQ(user,ustats,mbdn)	[expr [lindex $line end-4] / 1]
					
				}
				
				if { [string tolower [lindex $text 1]] == "day" } {
					
					set jTQ(ustats,stats)		"DAY"
					set jTQ(user,ustats,mbup)	[expr [lindex $line end-7] / 1]
					set jTQ(user,ustats,mbdn)	[expr [lindex $line end-6] / 1]
					
				}
				
				if { $jTQ(user,found) } {
		
					set output						[jTQ:FormatString $jTQ(skin,ustats,userstats)]
	
					if { $jTQ(default_show_ustats) == "chan" } 				{ jScripts:SND $chan $output }
					if { $jTQ(default_show_ustats) == "private" }			{ jScripts:SND $nick $output }
					if { $jTQ(default_show_ustats) == "notice" }				{ putquick "NOTICE $nick :$output" }
...
#############
# !gstats groupname all/mn/wk/day
#
#
proc jTQ:GSTATS {nick uhost hand chan text} {
  
	global jTQ jSB jScripts
	
	if { !$jTQ(enable_gstats) } { return }
	
	if { ([lindex $text 0] == "") || ([lindex $text 1] == "") } { 
		
		jScripts:SND $chan [jTQ:FormatString $jTQ(skin,gstats,help)]
		
		return 
		
	}

	if { $jTQ(enable_debug_partyline) } { putlog "..jTQ...: jTQ:GSTATS {$nick $uhost $hand $chan $text}" }
	
	foreach section $jTQ(stats_sections_locale) {
		
		set jTQ(user,ustats,mbup)	0
		set jTQ(user,ustats,mbdn)	0

		set n [split $section ':']
		
		set jTQ(group,found)	false
		...
		foreach line $data {
			...
			if { [lindex $line 1] == [lindex $text 0] } {
				...
				set jTQ(group,found)	true
				
				set jTQ(ustats,stats)		"ALL"
				#set jTQ(user,ustats,mbup)	[expr $jTQ(user,ustats,mbup) + [lindex $line end-1]]
				#set jTQ(user,ustats,mbdn)	[expr $jTQ(user,ustats,mbdn) + [lindex $line end]]
				...
				if { [string tolower [lindex $text 1]] == "mn" } {
					...
					set jTQ(ustats,stats)		"MN"
					set jTQ(user,ustats,mbup)	[expr $jTQ(user,ustats,mbup) + [lindex $line end-3]]
					set jTQ(user,ustats,mbdn)	[expr $jTQ(user,ustats,mbdn) + [lindex $line end-2]]
				...
				if { [string tolower [lindex $text 1]] == "wk" } {
					...
					set jTQ(ustats,stats)		"WK"
					set jTQ(user,ustats,mbup)	[expr $jTQ(user,ustats,mbup) + [lindex $line end-5]]
					set jTQ(user,ustats,mbdn)	[expr $jTQ(user,ustats,mbdn) + [lindex $line end-4]]
				...
				if { [string tolower [lindex $text 1]] == "day" } {
					...
					set jTQ(ustats,stats)		"DAY"
					set jTQ(user,ustats,mbup)	[expr $jTQ(user,ustats,mbup) + [lindex $line end-7]]
					set jTQ(user,ustats,mbdn)	[expr $jTQ(user,ustats,mbdn) + [lindex $line end-6]]
		...
		if { $jTQ(group,found) } {
			...
			set output						[jTQ:FormatString $jTQ(skin,gstats,groupstats)]
	...
	if { !$jTQ(group,found) } {
		...
		set jTQ(text_groupname) [lindex $text 0]
		
		set output						[jTQ:FormatString $jTQ(skin_group_not_found)]
...
proc jTQ:FormatString { tekst } {
	...
	set jTQ(text,usergroup)					"$jTQ(text_username)/$jTQ(text_groupname)"
	...
	for {set i 0} {$i < [string length $tekst]} {incr i} {
		...
		if {[string index $tekst $i] == "%"} {
			...
			switch -- [string index $tekst $i] {
				...
				{w}		{ append rpl [format %${x}.${y}s $jTQ(ustats,stats)] }				
			  	
				{A}		{ append rpl [format %${x}.${y}s $jTQ(text_out_passed)] }
				{B}		{ append rpl [format %${x}.${y}s $jTQ(text,usergroup)] }
				{C}		{ append rpl [format %${x}.${y}s $jTQ(user,ustats,mbup)] }
				{D}		{ append rpl [format %${x}.${y}s $jTQ(user,ustats,mbdn)] }
				{E}		{ append rpl [format %${x}.${y}s $jTQ(user,credits)] }
				{F}		{ append rpl [format %${x}.${y}s $jTQ(all,credits)] }
...
proc jTQ:QUOTACHECK {min hour day month year} {
	...
	set jTQ(enable_passed_end)			false
  	...
  	if { $jTQ(quota_check_interval) == "M" } {
		...
		set jTQ(year)							[clock format [clock seconds] -format "%Y" -gmt $jTQ(enable_gmt)]
		set jTQ(month)							[clock format [clock seconds] -format "%m" -gmt $jTQ(enable_gmt)]
		set jTQ(nmonth)						[expr int([format %f [clock format [clock seconds] -format "%m" -gmt $jTQ(enable_gmt)]])+1]
		if {$jTQ(nmonth) == 13} {
		  set jTQ(nmonth)						1
		  set jTQ(year)						[expr $jTQ(year)+1]
		}
		set jTQ(time_month_end)				[clock scan "$jTQ(nmonth)/01/$jTQ(year) 00:00:00" -gmt $jTQ(enable_gmt)]
	...
	set jTQ(quotas_nextmn_list)	""
  	...
  	if { $jTQ(enable_local) } {
			...
			if { [lsearch $jTQ(quota_excluded_users) [lindex $line 0]] == -1 } {
				...
				if { ([lsearch $jTQ(quota_excluded_groups) [lindex $line 1]] == -1) } {
					...
					if { ([regexp "[lindex $line 2]" "$jTQ(quota_excluded_flag)"]) == 0 } {
						...
						if { ([regexp "[lindex $line 2]" "$jTQ(quota_disable_flag)"]) == 0 } {
		...
		#quotas
			...
			if { $jTQ(enable,kick,on,endquota) } {
			
				foreach chan $jTQ(quotas,kick,chan,list) {
					
					#jScripts:SND $jTQ(default_admin_chan) $chan
					
					foreach user [chanlist $chan] {

						if {[isop $user $chan]} {
	
							#jScripts:SND $jTQ(default_admin_chan) "OP $user"
							
						} else {
						
							after $jTQ(time,kick,on,endquota)
							
							putkick $chan $user $jTQ(quotas,endofmonth,kick,msg)
...
proc jTQ:KICKALL {nick uhost hand chan text} {

	global jTQ jSB jScripts
	
	if { $jTQ(enable_debug_partyline) } { putlog "..jTQ...: jTQ:KICKALL {$nick $uhost $hand $chan $text}" }
...
proc jTQ:PASSED {nick uhost hand chan text} {
	...
	if { [lindex $text 0] == "" } { 
		...
		jScripts:SND $chan [jTQ:FormatString $jTQ(skin,passed,help)]
	...
	if { $jTQ(enable_local) } {
		...
		set jTQ(is_disabled) 			false								;#user is disabled
		...
		foreach section $jTQ(stats_sections_locale) {
			...
			if { $jTQ(is_found) } { 
				...
				if { $jTQ(is_trial) } {
					...
					#jScripts:SND $chan "Now.....: [clock format $jTQ(time_now) -format {%m/%d/%Y %H:%M:%S} -gmt $jTQ(enable_gmt) ]"
					#jScripts:SND $chan "Added...: $jTQ(text_dateadded)"
					#jScripts:SND $chan "Day.....: [clock format $jTQ(time_now) -format {%A} -gmt $jTQ(enable_gmt) ]"
					
					set jTQ(time_added)	[clock scan $jTQ(text_dateadded) -gmt $jTQ(enable_gmt)]
					...
					if { $jTQ(trial_check_interval) == "days" } {
				      ...
				      if { $jTQ(trial_to_end_days) == 0 } {
						...
						#jScripts:SND $chan "End.....: [clock format $jTQ(trial_end_time) -format {%m/%d/%Y %H:%M:%S} -gmt $jTQ(enable_gmt) ]"
						...
						if { $jTQ(trial_end_time) < $jTQ(time_now) } {
							...
							if { $jTQ(trial_failed_action) == "disable" } {
								...
								set status 	[catch { exec -- $jTQ(location_jscripts) ADDTRIALTOLOG $jTQ(text_username) $jTQ(trial_end_group) } result]
					
								if { $status > 0 } { 
									putlog "..jTQ...: $jTQ(location_jscripts) ADDTRIALTOLOG $jTQ(text_username) $jTQ(trial_end_group) FAiLED ($result)"
									jScripts:SND $jTQ(default_admin_chan) "..jTQ...: $jTQ(location_jscripts) ADDTRIALTOLOG $jTQ(text_username) $jTQ(trial_end_group) FAiLED ($result)"
									break
								}
				...
				#user is NOT in trial group
					...
					set jTQ(year)							[clock format [clock seconds] -format "%Y" -gmt $jTQ(enable_gmt)]
					set jTQ(month)							[clock format [clock seconds] -format "%m" -gmt $jTQ(enable_gmt)]
					set jTQ(time_month_begin)			[clock scan "$jTQ(month)/01/$jTQ(year) 00:00:00" -gmt $jTQ(enable_gmt)]
					set jTQ(nmonth)						[expr int([format %f [clock format [clock seconds] -format "%m" -gmt $jTQ(enable_gmt)]])+1]
					if {$jTQ(nmonth) == 13} {
					  set jTQ(nmonth)						1
					  set jTQ(year)						[expr $jTQ(year)+1]
			      ...
			      if { $jTQ(quota_to_end_days) == 0 } {
					...
					#jScripts:SND $chan "time now : [clock format $jTQ(time_now) -format {%m/%d/%Y %H:%M:%S} -gmt $jTQ(enable_gmt) ]"
					#jScripts:SND $chan "time mb  : [clock format $jTQ(time_month_begin) -format {%m/%d/%Y %H:%M:%S} -gmt $jTQ(enable_gmt) ]"
					#jScripts:SND $chan "time me  : [clock format $jTQ(time_month_end) -format {%m/%d/%Y %H:%M:%S} -gmt $jTQ(enable_gmt) ]"
		...
		if { $jTQ(is_trial) } {
			...
			set jTQ(end_date)		[clock format $jTQ(trial_end_time) -format {%m/%d/%Y %H:%M:%S} -gmt $jTQ(enable_gmt) ]
		...
		if { ([regexp "[lindex $line 2]" "$jTQ(quota_disable_flag)"]) != 0 } {
			...
			set jTQ(is_disabled) true
			
		}
	
		if { ( [clock scan $jTQ(text_dateadded) -gmt $jTQ(enable_gmt)] > $jTQ(time_month_begin) ) && ( !$jTQ(is_euser) && !$jTQ(is_egroup) && !$jTQ(is_eflag) ) } {
		...
		if { $jTQ(is_gquota) && !$jTQ(is_euser) && !$jTQ(is_eflag) } {
			...
			if { !$jTQ(enable_passed_quota) && $jTQ(enable_passed_end)} {
				...
				if { !$jTQ(is_euser) && !$jTQ(is_egroup) && !$jTQ(is_eflag) && !$jTQ(is_quota) } {
					...
					jScripts:LOG "$jScripts(log_dir)/[clock format [clock seconds] -format {%Y-%m-%d} -gmt $jTQ(enable_gmt)].$jTQ(log_name_quota_end)" "jTQ" [jTQ:FormatString "$jTQ(skin_quotas_nextmn_list) %15u/%-15g %y %-q"]
				...
				if { !$jTQ(is_euser) && !$jTQ(is_egroup) && !$jTQ(is_eflag) && $jTQ(is_quota) } {
					...
					if { $jTQ(gquota_passed) } {
						...
						jScripts:LOG "$jScripts(log_dir)/[clock format [clock seconds] -format {%Y-%m-%d} -gmt $jTQ(enable_gmt)].$jTQ(log_name_quota_end)" "jTQ" [jTQ:FormatString "$jTQ(skin_quotas_passed_list) %15u/%-15g %y %-q"]
						...
						if { $jTQ(group_quota_failed_action) == "nothing" } {
						...
						if { $jTQ(group_quota_failed_action) == "disable" } {
							...
							set status 	[catch { exec -- $jTQ(location_jscripts) ADDFLAG $jTQ(text_username) $jTQ(group_quota_disable_flag) } result]
				
							if { $status > 0 } { 
								putlog "..jTQ...: $jTQ(location_jscripts) ADDFLAG $jTQ(text_username) $jTQ(group_quota_disable_flag) FAiLED ($result)"
								jScripts:SND $jTQ(default_admin_chan) "..jTQ...: $jTQ(location_jscripts) ADDFLAG $jTQ(text_username) $jTQ(group_quota_disable_flag) FAiLED ($result)"
								break
							}
						...
						if { $jTQ(group_quota_failed_action) == "delete" } {
						...
						if { $jTQ(group_quota_failed_action) == "trial" } {
						...
						if { $jTQ(group_quota_failed_action) == "credits" } {
							...
							set status 	[catch { exec -- $jTQ(location_jscripts) CHANGECREDITS $jTQ(text_username) -$jTQ(group_quota_fail_remove_credits) $jTQ(group_quota_fail_remove_in_section) } result]
				
							if { $status > 0 } { 
								putlog "..jTQ...: $jTQ(location_jscripts) CHANGECREDITS $jTQ(text_username) -$jTQ(group_quota_fail_remove_credits) $jTQ(group_quota_fail_remove_in_section) FAiLED ($result)"
								jScripts:SND $jTQ(default_admin_chan) "..jTQ...: $jTQ(location_jscripts) CHANGECREDITS $jTQ(text_username) -$jTQ(group_quota_fail_remove_credits) $jTQ(group_quota_fail_remove_in_section) FAiLED ($result)"
								break
							}
						...
						jScripts:LOG "$jScripts(log_dir)/[clock format [clock seconds] -format {%Y-%m-%d} -gmt $jTQ(enable_gmt)].$jTQ(log_name_quota_end)" "jTQ" [jTQ:FormatString "$jTQ(skin_endquota_failed_head) %15u/%-15g %y %-q"]
			...
			if { !$jTQ(enable_passed_quota) && !$jTQ(enable_passed_end)} {
				...
				if { $jTQ(is_passed) } {
					...
					if { $jTQ(is_disabled) } {
						...
						set output		$jTQ(skin,quota,out,account,disabled)
						#set output		[jScripts:Replace $output "%status" $jTQ(text_out_passed) ]
					...
					if { !$jTQ(is_euser) && !$jTQ(is_egroup) && !$jTQ(is_eflag) && $jTQ(is_quota) && !$jTQ(is_disabled)} {
...
proc jTQ:UPDATE {min hour day month year} {
	...
	if { $jTQ(enable_local) } {
		...
		foreach section $jTQ(stats_sections_locale) {
			...
			if { $jTQ(enable_update_anounce) } { jScripts:SND $jTQ(default_admin_chan)	"..jTQ...: Updating Quota Stats" }
